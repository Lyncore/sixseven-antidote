...
<template>
  <section class="card">
    <div class="path-row">
      ...
      <button @click="emit('pickFolder')" class="btn-secondary" :disabled="scanning">
        {{ t("browse") }}
      </button>
    </div>
    ...
    <div class="options-row">
      <label class="checkbox-label">
        <input type="checkbox" v-model="recursive" :disabled="scanning" /> {{ t("recursive") }}
      </label>
      <label class="checkbox-label">
        <input type="checkbox" v-model="backup" :disabled="scanning" /> {{ t("backups") }}
      </label>
    </div>
    ...
    <div v-if="scanning" class="progress-wrap">
      ...
      <div class="progress-info">
        ...
        <span v-if="progress" class="progress-current">{{ progress.current }}</span>
      </div>
    </div>
  </section>
</template>
